...
function EditPageContent() {
  ...
  const fetchSlipData = async () => {
    try {
      const res = await fetch(`/api/salary-slips?id=${slipId}`);
      const data = await res.json();
      if (data.success && data.data) {
        // Remove _id and other MongoDB fields before setting state
        const { _id, createdAt, __v, ...slipData } = data.data;
        setSalaryData(slipData as SalaryDetails);
      } else {
      ...
      alert("Failed to load slip data");
      router.push("/dashboard");
    } finally {
      setIsLoading(false);
    }
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      const canvas = await html2canvas(salarySlipRef.current, {
        ...
        windowHeight: salarySlipRef.current.scrollHeight + 100,
      });
